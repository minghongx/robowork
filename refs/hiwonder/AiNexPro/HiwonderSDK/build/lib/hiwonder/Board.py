...
def initPWMServo(d):
    ...
    servo1 = PWMServo.PWM_Servo(pi, 6, deviation=d[0], control_speed=True)  # 扩展板上的1
    servo2 = PWMServo.PWM_Servo(pi, 12, deviation=d[1], control_speed=True)  # 2
...
def setBusServoID(oldid, newid):
    ...
    :param newid: 新的id
    """
    BusServoCmd.serial_servo_write_cmd(oldid, BusServoCmd.LOBOT_SERVO_ID_WRITE, newid)
...
def setBusServoPulse(id, pulse, use_time, send_again=True):
    """
    驱动串口舵机转到指定位置
    :param id: 要驱动的舵机id
    :pulse: 位置
    ...
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_MOVE_TIME_WRITE, pulse, use_time)
    if send_again:
        time.sleep(0.003)
        BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_MOVE_TIME_WRITE, pulse, use_time)
...
def stopBusServo(id=None):
    ...
    :return:
    '''
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_MOVE_STOP)
...
def setBusServoDeviation(id, d=0):
    ...
    :param d:  偏差
    """
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_ANGLE_OFFSET_ADJUST, d)
...
def saveBusServoDeviation(id):
    ...
    :param id: 舵机id
    """
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_ANGLE_OFFSET_WRITE)
...
def setBusServoAngleLimit(id, low, high):
    ...
    :return:
    '''
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_ANGLE_LIMIT_WRITE, low, high)
...
def setBusServoVinLimit(id, low, high):
    ...
    :return:
    '''
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_VIN_LIMIT_WRITE, low, high)
...
def setBusServoMaxTemp(id, m_temp):
    ...
    :return:
    '''
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_TEMP_MAX_LIMIT_WRITE, m_temp)
...
def restBusServoPulse(oldid):
    ...
    time.sleep(0.1)
    BusServoCmd.serial_servo_write_cmd(oldid, BusServoCmd.LOBOT_SERVO_MOVE_TIME_WRITE, 500, 100)    # 中位

##掉电
def unloadBusServo(id):
    BusServoCmd.serial_servo_write_cmd(id, BusServoCmd.LOBOT_SERVO_LOAD_OR_UNLOAD_WRITE, 0)
